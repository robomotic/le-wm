...
def _merge(path_a: Path, path_b: Path, out: Path) -> None:
    ...
    with h5py.File(path_a, "r") as fa, \
        ...
        # Recompute offsets for the combined dataset
        ...
        for k in data_keys:
            fo.create_dataset(k, data=np.concatenate([fa[k][:], fb[k][:]], axis=0))

    n_ep = len(ep_lens)
    n_steps = n_steps_a + n_steps_b
    print(f"Merged → {out}  ({n_ep} episodes, {n_steps} steps)")
